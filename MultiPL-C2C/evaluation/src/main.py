...
def cached_eval_script(problem, index) -> dict:
    ...
    # pdb.set_trace()
    if program.startswith("### Translate"):
        start_str = f"### {problem['language']} version\n\n"
        program = program[program.find(start_str)+len(start_str):]
    elif program.startswith("### original python code"):  # after adding comments
        replace_str = program[program.find("def "):program.find("### commented python code") + 26]
        program = program.replace(replace_str, "")
        candidate_name = program[program.find("def ")+4:program.find("(")].strip()
...
def evaluate_problem(output_dir: Path, problem_json_path: Path, max_workers: int, input_dir: Path = None):
    with open_json(problem_json_path, "r") as f:
        try:
            ...
            print(f"Corrupt completion file: {f}")
            return
    ...
    if args.file:
        ...
        for file in tqdm(files):
            ...
            try:
                ...
                print(f"evaluation error on {file}, skip for now: {e}")
                continue
