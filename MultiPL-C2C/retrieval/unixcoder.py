...
class UniXcoder(nn.Module):
    ...
    def tokenize(self, inputs, mode="<encoder-only>", max_length=512, padding=None):
        """
        ...
        * `padding`- whether to pad source sequence length to max_length. [max, batch, None]
        * `mode`- which mode the sequence will use. i.e. <encoder-only>, <decoder-only>, <encoder-decoder>
        ...
        if padding == "batch":
            batch_max = max(len(t) for t in tokens_ids)
            for i, tokens_id in enumerate(tokens_ids):
                if len(tokens_id) < batch_max:
                    tokens_ids[i] = tokens_id + [self.config.pad_token_id] * (batch_max - len(tokens_id))
