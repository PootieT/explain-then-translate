...
class JavaProcessor(TreeSitterLangProcessor):
    ...
    def obfuscate_code(self, code):
        if "class" not in code.split()[:3]:
            try:
                # when just obfusecating a class method, it throws an error,
                # so we wrap it in dummy class, and remove wrapper later
                code = "public class dummyclass {\n" + code + "\n}\n"
                res, dico = javalang_obfuscator.obfuscate(code)
                res = res.replace("\npublic class CLASS_0 {\n","")[:-2]
            except Exception as e:
                # print(f"exception obfuscating java function: \n{code}\nerror:\n{e}")
                res, dico = code, {}
        else:
            res, dico = javalang_obfuscator.obfuscate(code)
