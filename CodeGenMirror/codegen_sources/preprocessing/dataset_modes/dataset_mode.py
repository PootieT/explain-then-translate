...
class DatasetMode(Generic[T]):
    ...
    def extract_data_and_tokenize(
        ...
        pbar = tqdm.tqdm(total = len(json_files), desc="Extracting and tokenizing")
        json_file_chunks = chunks(json_files, MAX_ARRAY_SIZE_CLUSTER)
        # pdb.set_trace()
        for i, json_files in enumerate(json_file_chunks):
            pbar.set_description(f"On chunk {i}, processing and tokenizing {len(json_files)} files")
        ...
        for f in chain(
            *[
            ...
            if not is_valid_file(f):
                ...
                jobs_files.append(f)
        jobs_files_chunks = chunks(jobs_files, MAX_ARRAY_SIZE_CLUSTER)
        pbar = tqdm.tqdm(total=len(jobs_files), desc="Apply BPE to files in chunks")
        for i, jobs_files_chunk in enumerate(jobs_files_chunks):
            jobs = []
            for f in jobs_files_chunk:
                logger.info(f"Applying BPE on {f} ...")
                jobs.append(executor.submit(self.bpe.apply_bpe_file, f, f.with_suffix(self.bpe.ext)))
            for job in jobs:
                job.result()
                pbar.update(1)
                pbar.set_description(f"Processing chunk {i} in applying BPE to files")
        ...
        for f in chain(
            *[
            ...
            if not is_valid_file(f.with_suffix(f.suffix + ".pth")):
                jobs_files.append(f)
        jobs_files_chunks = chunks(jobs_files, MAX_ARRAY_SIZE_CLUSTER)
        pbar = tqdm.tqdm(total=len(jobs_files), desc="Binarize files in chunks")
        for i, jobs_files_chunk in enumerate(jobs_files_chunks):
            jobs = []
            for f in jobs_files_chunk:
                logger.info(f"Binarizing {f} ...")
                jobs.append(executor.submit(binarize_for_XLM_file, f, self.bpe.vocab_path))
            for job in jobs:
                job.result()
                pbar.update(1)
                pbar.set_description(f"Processing chunk {i} in binarizing files")
