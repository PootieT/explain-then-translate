...
def compute_all_tests_results(tests, functions, test_runner, output_path=None):
    executor = ProcessPoolExecutor(240)
...
def translate_tests(java_tests, translator):
    ...
    print(f"======== inside translate_tests: {len(java_tests)} java tests")
...
def compute_test_results(
    ...
    input_df = input_df.drop_duplicates(subset=["TARGET_CLASS"])
    input_df = input_df.drop_duplicates(subset=["java_function"])
    if target_language == "python":
        test_runner = PythonTestRunner(timeout=180)
        test_translator = EvosuiteToPython()
    else:
        assert target_language == "cpp"
        test_runner = CppTestRunner(compilation_timeout=180)  # originally 30 sec
    ...
    # job = cluster.submit(
    #     translate_tests, input_df.tests_strings.values, test_translator
    # )
    # translated_tests = job.result()
    ...
    CHUNKSIZE_TEST_RESULTS = len(translated_tests) // NUM_SPLITS
    ...
    # the chunks function can return a small last chunk that we can just add to the second-to-last chunk
    if len(tests_chunks) > NUM_SPLITS:
        tests_chunks = tests_chunks[:-2] + [tests_chunks[-2] + tests_chunks[-1]]
        func_chuncs = func_chuncs[:-2] + [
            np.concatenate((func_chuncs[-2], func_chuncs[-1]), axis=0)
        ]
...
def parse_arguments():
    ...
    parser.add_argument(
        "--target_language",
        help="target language. python or cpp",
        default="python"
    )
