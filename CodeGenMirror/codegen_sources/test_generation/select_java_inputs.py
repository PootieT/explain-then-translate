...
if __name__ == "__main__":
    ...
    tok_files_chunked = list(chunks(tok_files, MAX_ARRAY_SIZE_CLUSTER))
    outer_pbar = tqdm(total = len(tok_files), desc="Processing files")
    for i, tok_files_chunk in enumerate(tok_files_chunked):
        jobs = cluster.map_array(
            select_functions_for_file,
            tok_files_chunk,
            repeat(output_path),
        )
        for j in jobs:
            j.result()
            outer_pbar.update(1)
            outer_pbar.set_description(f"Processing files, on chunk {i} of {len(tok_files_chunked)}")
    # these lines are from the old implementation with no array limits
    ...
    deduped_funcs_chunks = chunks(deduped_funcs, CHUNKSIZE)
