...
from .data.loader import SELF_TRAINED, TRAIN_SPLITS
from .model.CustomDDP import CustomTorchDDP, CustomApexDDP   # TODO temporary local debug
...
from .utils import (
    ...
    get_programming_language_name, get_mixed_langs_id, shift_paddings_for_qe,
)
...
SPECIAL_SEP_INDEX = 4  # index of <special0>, token used to separate different language programs

class Trainer(object):
    def __init__(self, data, params, model_names, initialize_optim=True):
        """
        Initialize trainer.
        """
        ...
        # load the attention matrix for supervision
        self.load_att_mtx(params.attn_mask_mtx)

        if initialize_optim:
            # float16 / distributed (no AMP)
            assert params.amp >= 1 or not params.fp16
            assert params.amp >= 0 or params.accumulate_gradients == 1
            MODEL_NAMES = self.MODEL_NAMES + ["qe_model"] if params.st_qe_model_path else self.MODEL_NAMES
            ...
            if params.amp >= 0:
                self.init_amp()  # TODO temporary local debug
                if params.multi_gpu:
    ...
    #                )
        ...
        if len(params.st_steps) > 0 and not params.st_qe_model_path:
            self.test_runners = {
        ...
        self.stats = OrderedDict(
            ...
            + [("MT-%s-%s-%s" % (s, l1, l2), []) for l1, l2 in params.mt_steps for s in params.train_splits if s != "train"]
            + [("MT-ATT-%s-%s" % (l1, l2), []) for l1, l2 in params.mt_steps] # added attention step / loss
            + [
                ("MT-%s-%s-%s" % (l1, l2, span), [])
            ...
            + [("Classif-%s-%s-%s" % (l1, l2, label), []) for l1, l2, label in params.seq_classif_steps]
        ...
        # inititailize wandb logging
        if params.log_wandb and params.is_master:
            self.init_wandb()

    def init_wandb(self):
        wandb.init(
            project="CodeGen",
            name=f"{self.params.exp_name}",
            settings=wandb.Settings(start_method="fork") if torch.cuda.is_available() else None,
        )
        safe_params = {k: str(v) if isinstance(v, tuple) else v for k, v in self.params.__dict__.items()
                       if type(v) in [int, bool, None, float, str]}
        wandb.config.update(safe_params)

    def load_att_mtx(self, load_att_mtx):
        if load_att_mtx:
            assert os.path.isfile(load_att_mtx)
            self.att_mtx = to_cuda(torch.load(load_att_mtx))[0]
            assert self.att_mtx.shape[0] == 6, f"had {self.att_mtx.shape[0]} layers instead of 6" # layers
            assert self.att_mtx.shape[1] == 8, f"had {self.att_mtx.shape[1]} heads instead of 8" # heads
            print("Loaded attention matrix")
            print(self.att_mtx)
        else: 
            self.att_mtx = None
    ...
    def set_parameters(self):
        ...
        for name in self.MODEL_NAMES:
            ...
            if self.params.freeze_encoder and name == "model":
                continue
    ...
    def init_amp(self):
        ...
        OPT_MODEL_NAMES = ["classifier"] if params.freeze_encoder else self.MODEL_NAMES
        models = [
            model
    ...
    def iter(self):
        ...
        if self.n_iter % self.params.log_steps == 0:  # TODO only in master node?
            if self.params.log_wandb and self.params.is_master:
                self.log_wandb()
            self.print_stats()
    ...
    def log_wandb(self, **kwargs):
        if len(kwargs) > 0:
            ...
            val_dict = {k: np.mean(v) for k, v in self.stats.items() if type(v) is list and len(v) > 0}
            if len(self.st_translation_stats) > 0:
                flat_st_stats = {f"{l}_{k}": v for l, l_stat in self.st_translation_stats.items() for k, v in l_stat.items()}
                val_dict.update(flat_st_stats)
        val_dict["Steps"] = self.n_total_iter
        val_dict["epoch"] = self.epoch
        val_dict["n_sentences"] = self.n_sentences
        val_dict["n_words"] = self.stats["processed_s"]
        val_dict["learning_rate"] = list(self.optimizers.values())[0].param_groups[0]["lr"]
        wandb.log(val_dict)
    ...
    def get_iterator(
        ...
        splt = SELF_TRAINED if self_training else dataset if dataset is not None else list(TRAIN_SPLITS)[0]
        logger.info(
            "Creating new training data iterator (%s) ..."
            % ",".join([str(x) for x in [iter_name, dataset, lang1, lang2] if x is not None])
        )
        if lang2 is None:
            if stream:
                if span is None and label is None:
                    ...
                    iterator = self.data["mono_stream"][(lang1, label)][splt].get_iterator(
                        shuffle=True
                    )
                else:
            ...
            _lang1, _lang2 = (lang1, lang2) if (lang1 < lang2 or label is not None) else (lang2, lang1)
            it = (
                self.data["para"][(_lang1, _lang2, span)][splt]
                if span is not None
                else self.data["para"][(_lang1, _lang2, label)][splt]
                if label is not None
                else self.data["para"][(_lang1, _lang2)][splt]
            )
            iterator = it.get_iterator(
                shuffle=True,
                group_by_size=self.params.group_by_size,
        ...
        key = (
            (iter_name, lang1, lang2, label)
            if label is not None
            else (iter_name, lang1, lang2, span)
            if span is not None
            else (iter_name, lang1, lang2, dataset)
            if dataset is not None
            else (iter_name, lang1, lang2)
        )
    ...
    def get_batch(
        ...
        iterator = (
            ...
            self.iterators.get((iter_name, lang1, lang2, label), None)
            if label is not None else
            self.iterators.get((iter_name, lang1, lang2, dataset), None)
            if dataset is not None else
            self.iterators.get((iter_name, lang1, lang2), None)
        )
        if (
            st_scores_cutoff
        ...
        x = x if lang2 is None or lang1 < lang2 or label is not None else x[::-1]
        # if there is dataset prefix, remove the prefix for target sequences
        # TODO temp maybe sketchy code
        if self.params.add_dataset_prefix:
            x2 = torch.cat([x[1][0][[0]], x[1][0][2:]])  # remove prefix from target sentence
            len2 = x[1][1] - 1  # decrease target sentence len
            x = (x[0], (x2, len2, x[1][2], x[1][3]))
        return x
    ...
    def classif_step(self, lang1, lang2, lambda_coeff, label=None, eval_only=False, model=None, x1=None, len1=None, y=None, len2=None):
        """
        classification (per token or sequence level classification)
        if label is present, it's sequence level classification
        if eval_only, it's used as forward pass to obtain sequence classification results for self-training (QE)
        """
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        ...
        if model is None:
            ...
            # half it or something
        lang1_id = params.lang2id[lang1]
        if (label is not None) or (eval_only):
            if x1 is None:
                # y is 3 X n shaped, where middle row contains the class index
                (x1, len1, _, _), (y, len2, _, _) = self.get_batch("classif", lang1, lang2, label=label)
            # pred_mask = x1 == self.params.eos_index  # 2 eos tokens persentence, just take first one
            pred_mask = torch.zeros(x1.shape[0], x1.shape[1]).bool()
            pred_mask[0, :] = True
            assert all(len2==3)  # labels should not be tokenized, and should have only one label dim for seq classif
            lang2_id = params.lang2id[lang2]
            langs1 = get_mixed_langs_id(x1, lang1_id, lang2_id, SPECIAL_SEP_INDEX)
        else:
        ...
        # cuda
        x1, len1, langs1, y = to_cuda(x1, len1, langs1, y)  # TODO temporary local debug
        # encode source sentence
        enc1 = model("fwd", x=x1, lengths=len1, langs=langs1, causal=False)
        if self.params.fp16:
            enc1 = enc1.half()
            if classifier.proj.weight.dtype != torch.float16:  # TODO temporary local debug (and for loading QE model)
                classifier = classifier.half()

        # classification + loss
        scores, loss = classifier(enc1, y, pred_mask)
        loss_name = f"Classif-{lang1}-{lang2}" if label is None else f"Classif-{lang1}-{lang2}-{label}"
        if eval_only:
            if loss_name not in self.stats:
                self.stats[loss_name] = [loss.item()]
            else:
                self.stats[loss_name].append(loss.item())
            return scores
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(
        ...
        lang2_id = params.lang2id[lang2]
        decoder = (
            self.decoder[lang2_id] if params.separate_decoders else
            self.decoder[params.train_splits.index(train_dataset)] if params.separate_dataset_decoders and train_dataset is not None else
            self.decoder[0]
        )
        ...
        elif att_supervision:
            # use only x2_attn as get_batch reverses depending on direction
            ((x1, len1, _, _), (_, _)),  ((x2, len2, _, _), (x2_attn, x2_attn_mask)) = self.get_batch(
                "mt", lang1, lang2
            )
            x2_attn, x2_attn_mask = to_cuda(x2_attn, x2_attn_mask)
        else:
            (x1, len1, _, _), (x2, len2, _, _) = self.get_batch("mt", lang1, lang2, dataset=train_dataset)
        ...
        if att_supervision:
            att_loss = decoder("attention_loss", target_scores=x2_attn,
                               target_mask=x2_attn_mask,
                               loss_type="mse",
                               broadcast_mask=self.att_mtx)
            loss += lambda_att * att_loss
            if lang1 != lang2:
                key = (lang1, lang2) if span is None else (lang1, lang2, span)
                self.stats[("MT-ATT-%s" % "-".join(key))].append(att_loss.item())
        ...
        if deobfuscate:
            ...
            key = (lang1, lang2) if span is None else (lang1, lang2, span)
            self.stats[
                f"AE-{lang1}" if lang1 == lang2 else
                f"MT-{train_dataset}-{'-'.join(key)}" if train_dataset != "train" else
                f"MT-{'-'.join(key)}"
            ].append(loss.item())
        loss = lambda_coeff * loss
    ...
    def st_step(self, lang1, langs2, lambda_coeff, show_example=False):
        ...
        if read_from_cache:
            ...
            (x1, len1, id1, lenid1) = self.get_batch(
                ...
                lang1,
                self_training=True,
                st_scores_cutoff=None if params.st_qe_model_path else (
    ...
    def generate_parallel_examples(
        ...
        with torch.no_grad():
            ...
            if params.st_qe_model_path:
                test_outputs = self.get_qe_outputs(x1, len1, x2, len2, lang1, lang2)
            else:
    ...
    def log_successful_st(self, any_successful, key):
        if key not in self.st_translation_stats:
            self.st_translation_stats[key] = {"success": 0, "failure": 0}
        self.st_translation_stats[key]["success"] += sum(any_successful)
        self.st_translation_stats[key]["failure"] += len(any_successful)
        total = self.st_translation_stats[key]["success"] + self.st_translation_stats[key]["failure"]
        self.st_translation_stats[key]["success_pct"] = self.st_translation_stats[key]["success"] / total
        self.st_translation_stats[key]["failure_pct"] = self.st_translation_stats[key]["failure"] / total
        if (
            ...
            #     f"{self.st_translation_stats[key]['success'] / self.st_translation_stats[key]['failure']:.2%}"
            #     f" ({self.st_translation_stats[key]['success']} / {self.st_translation_stats[key]['failure']})"
            # )
            logger.info(
                f"Percentage of successful translations {key}: "
                f"{self.st_translation_stats[key]['success_pct'] :.2%}"
                f" ({self.st_translation_stats[key]['success']} / {total})"
            )
    ...
    def get_qe_outputs(self, x1, len1, x2, len2, lang1, lang2):
        """
        x1 has shape (s_len, bs), len1 has shape (bs)
        x2 has shape (s_len, n_beams, bs), len2 has shape (bs, n_beams)
        """
        s_len, n_beams, bs = x2.shape

        x1 = torch.repeat_interleave(x1, n_beams, dim=1)
        len1 = torch.repeat_interleave(len1, n_beams)
        x2 = x2.transpose(1,2).reshape(s_len, -1)
        len2 = len2.reshape(-1)
        sep_id, = to_cuda(torch.ones(1, bs*n_beams, dtype=x1.dtype) * SPECIAL_SEP_INDEX)

        # can't just simply cat, need to shift paddings from end of x1 to end of x2
        x3 = torch.cat((x1, sep_id, x2), dim=0)
        x3 = shift_paddings_for_qe(x1, x3, self.params.pad_index, SPECIAL_SEP_INDEX)
        lenx = len1 + len2 + 1

        y = torch.zeros(3, bs*n_beams, dtype=x1.dtype, device=x1.device)
        leny = torch.ones(bs*n_beams, dtype=len1.dtype, device=x1.device) * 3
        model_idx = self.params.st_steps[0][1].index(lang2)
        qe_model = self.qe_model[model_idx]
        # need to batch it if original batch size it's too large (X20 factor here)
        BS = 400 if s_len < 100 else 100  # this has to be divisible by beam size (20)
        num_batches = (n_beams* bs) // BS
        if n_beams * bs > BS*num_batches:
            num_batches += 1
        logits_agg = []
        for i in range(num_batches):
            logits = self.classif_step(
                lang1, lang2, 1, label="qe_label", eval_only=True, model=qe_model,
                x1=x3[:, i*BS:i*BS+BS], len1=lenx[i*BS:i*BS+BS],
                y=y[:, i*BS:i*BS+BS], len2=leny[i*BS:i*BS+BS]
            )
            logits = logits.reshape(logits.shape[0]//n_beams, n_beams, 2)
            logits_agg.append(logits)
        logits = torch.cat(logits_agg, 0)

        res = []
        for s in range(bs):
            beam_res = []
            for b in range(n_beams):
                if (logits[s,b,1] - logits[s,b,0]) > self.params.st_min_qe_score:
                    beam_res.append(["success"])
                else:
                    beam_res.append(["failure"])
            res.append(beam_res)
        return res


class CustomPPOTrainer(Trainer, PPOTrainer):
    def __init__(self, encoder, decoder, data, params, second_decoder=None, qe_model=None):
        ...
        # inherit methods from Trainer (mostly for dataset iterators)
        Trainer.__init__(self, data, params, self.MODEL_NAMES, initialize_optim=False)

        # initialize as PPOTrainer (wrapped model, ref model, tokenizer, optimizer)
        # can define other args through trainer args later
        ppo_config = PPOConfig(
            learning_rate=self.params.,
            batch_size=1, # this will be dynamically changed throughout training
            log_with="wandb" if self.params.log_wandb else None,
            tracker_project_name="CodeGen",
            max_grad_norm=self.params.clip_grad_norm,
        )
        PPOTrainer.__init__(self, ppo_config, model, ref_model, tokenizer)
