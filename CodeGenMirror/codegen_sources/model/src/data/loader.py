...
from data.alignment.Aligner import Aligner
from ..utils import copy_over_training_data_files, combine_train_splits

SELF_TRAINED = "self_training"
# DATASET_SPLITS = ["train"] # QE training # "test_st"]
DATASET_SPLITS = ["train"]  # "valid", "test", translation training
TRAIN_SPLITS = {"train", SELF_TRAINED}  # {"train", SELF_TRAINED} # TODO verify self-training split is not necessary
LABEL_NAMES = {"qe_label"}
...
def load_binarized(path, params):
    ...
    if getattr(params, "multi_gpu", False):
        ...
        if os.path.isfile(split_path):
            assert params.split_data is False, split_path
            path = split_path
...
def load_matrices(attn_path, params):
    """
    Load matrices from disk.
    """
    attentions = json.load(open(attn_path))
    pbar = tqdm(total=len(attentions), desc="Loading attention matrices")
    attention_matrices = []
    for attn in attentions:
        # todo build a different matrix using different methods
        attn_matrix = Aligner.alignment_matrix_from_alignment_list(attn)
        attention_matrices.append(attn_matrix)
        pbar.update(1)
    return attention_matrices


def load_labels(tgt_path: str, tgt_data: Dict) -> Tuple[Dict, np.array]:
    class_indices = np.array([int(l.strip()) for l in open(tgt_path.replace(".pth", ".bpe"), "r").readlines()]).astype(int)
    n = len(class_indices)
    positions = np.arange(n*2).reshape(n, 2)
    sentences = np.vstack([class_indices, np.ones(n)]).T.reshape(n*2)
    tgt_data["positions"] = positions
    tgt_data["sentences"] = sentences
    return tgt_data, class_indices
...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt, data_path in params.mono_dataset[lang].items():
            ...
            bs = params.batch_size if splt in TRAIN_SPLITS else 1
            ...
            if (
                ...
                if splt == SELF_TRAINED and not params.st_qe_model_path:
...
def load_para_data(params, data):
    ...
    for key in params.para_dataset.keys():
        ...
        if span is not None:
            ...
            logger.info("============ Parallel data (%s-%s-%s)" % (src, tgt, label))
        else:
        ...
        # if len(params.train_splits) > 0:
        #     combine_train_splits(params)
        ...
        for splt in DATASET_SPLITS:
            ...
            if splt in TRAIN_SPLITS and (
                ...
                or (
                    label is not None
                    and (src, tgt, label) not in required_para_train
                    and (tgt, src, label) not in required_para_train
                )
                or (
                    span is None and label is None
                    and (src, tgt) not in required_para_train
                    and (tgt, src) not in required_para_train
                )
            ):
                print("back translation can't load training data!")
                continue
            ...
            if splt in TRAIN_SPLITS and params.attention_supervision:
                # pdb.set_trace()
                src_attn_pth, tgt_attn_pth = params.para_dataset[key]["train_attn"]
                src_attn = load_matrices(src_attn_pth, params)
                tgt_attn = load_matrices(tgt_attn_pth, params)
                attn_matrices = (src_attn, tgt_attn)
            else:
                attn_matrices = None
            ...
            if label is not None:
                tgt_data, labels = load_labels(tgt_path, tgt_data)  # load BPE token as class index (not indexed BPE token)
                if splt in TRAIN_SPLITS:
                    classes = range(int(labels.max()+1))  # expect class to be indexed from 0 to class size
                    params.class_weights = [1-sum(labels==c)/len(labels) for c in classes]
            ...
            if span is not None:
                ...
                data["para"][(src, tgt, label)][splt] = dataset
            else:
...
def check_data_params(params):
    ...
    assert os.path.isdir(params.wild_data_path) or not params.wild_data_path, params.wild_data_path

    ## modifying training splits
    params.train_splits = params.train_splits.split(",") if params.train_splits else []

    # if wild data is specified, copy over the data (translation finetuneing only)
    if params.wild_data_path and not params.classif_steps:
        # if not any(f.startswith("train") for f in os.listdir(params.wild_data_path)):
        copy_over_training_data_files(params)
        params.data_path = params.wild_data_path

    # for QE datasets only, we can combine them. TODO combine train_splits use-caes for QE and translation
    if len(params.train_splits) > 0:
        if params.classif_steps:
            DATASET_SPLITS.remove("train")
            DATASET_SPLITS.append("train_combined")
            TRAIN_SPLITS.remove("train")
            TRAIN_SPLITS.add("train_combined")
            combine_train_splits(params)
        else:
            DATASET_SPLITS.extend([s for s in params.train_splits if s not in DATASET_SPLITS])
            TRAIN_SPLITS.update(params.train_splits)
    ...
    params.seq_classif_steps = [
        tuple(s.split("-")) for s in params.seq_classif_steps.split(",") if len(s) > 0
    ]
    for i, step in enumerate(params.seq_classif_steps):
        if step[1].lower == "none":
            params.seq_classif_steps[i] = None
    assert all([len(x) == 3 for x in params.seq_classif_steps])
    assert all([(l1 in params.langs and l2 in params.langs) for l1, l2, label in params.seq_classif_steps])
    assert all([l1 != l2 for l1, l2, label in params.seq_classif_steps])
    assert len(params.seq_classif_steps) == len(set(params.seq_classif_steps))
    assert (
            len(params.seq_classif_steps) + len(params.mt_spans_steps) == 0
            or not params.n_classes_classif <= 0
    )
    params.use_classifier = True if (len(params.classif_steps) > 0) or (len(params.seq_classif_steps) > 0) else False
    ...
    # add eval and train splits if there are specified in the args
    params.eval_splits = params.eval_splits.split(",") if params.eval_splits else []
    DATASET_SPLITS.extend(params.eval_splits)
    ...
    ## load in the paths to the attention alignments
    if params.attention_supervision:
        # pdb.set_trace()
        for src in params.langs:
            for tgt in params.langs:
                if src < tgt and ((src, tgt) in required_para or (tgt, src) in required_para):
                    # src_pth, tgt_pth = params.para_dataset[(src, tgt)]["train"]
                    src_attn_pth = os.path.join(
                        params.data_path,
                        "train.%s-%s.%s.alignments.json" % (src, tgt, src)
                    )
                    tgt_attn_pth = os.path.join(
                        params.data_path,
                        "train.%s-%s.%s.alignments.json" % (src, tgt, tgt)
                    )
                    assert os.path.isfile(src_attn_pth) and os.path.isfile(tgt_attn_pth)
                    params.para_dataset[(src, tgt)]["train_attn"] = (
                        src_attn_pth,
                        tgt_attn_pth,
                    )
                    #
                    # params.para_dataset[(src, tgt)]["train"] = (src_pth, tgt_pth, src_attn_pth, tgt_attn_pth)
    ...
    for lang1, lang2, label in params.seq_classif_steps:
        # expect first of tuple to be concactenated lang1+lang2 or just lang1 sequence
        params.para_dataset[(lang1, lang2, label)] = {
            splt: (
                os.path.join(
                    params.data_path, "%s.%s-%s.%s.pth" % (splt, lang1, lang2, lang1)
                ),
                os.path.join(
                    params.data_path, "%s.%s-%s.%s.pth" % (splt, lang1, lang2, label)
                ),
            )
            for splt in DATASET_SPLITS
        }
    ...
    # check that all evaluation splits are loaded in data
    if len(params.eval_splits) > 0:
        # if len(params.mono_dataset) > 0:
        #     for datasets in params.mono_dataset.values():
        #         assert all(splt in datasets.keys() for splt in params.eval_splits)
        if len(params.para_dataset) > 0:
            for datasets in params.para_dataset.values():
                assert all(splt in datasets.keys() for splt in params.eval_splits)
