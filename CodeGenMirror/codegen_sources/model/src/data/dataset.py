...
class Dataset(object):
    ...
    def batch_matrices(self, matrices):
        """
        Create mini-batches of matrices, each a n x m tensor
        Stack and pad to form a b x n x m tensor.
        """
        bs = len(matrices)
        max_d0 = max(m.shape[0] for m in matrices)
        max_d1 = max(m.shape[1] for m in matrices)
        x = torch.LongTensor(bs, max_d0, max_d1).fill_(0.0)
        position_masks = torch.LongTensor(bs, max_d0, max_d1).fill_(0.0)
        for i, m in enumerate(matrices):
            d1, d2 = m.shape
            x[i, :d1, :d2] = m
            position_masks[i, :d1, :d2] = 1.0
        ## add masks -> 0 for outside, 1 inside
        return x, position_masks
...
class ParallelDataset(Dataset):
    def __init__(
        ...
        The length of the lists should be 2 when doing translation or span classification
         or sequence classification (w/ label) and 3 when doing translation using spans
        """
        ...
        self.has_attention = attention_matrices not in (None, [])
        if self.has_attention:
            self.attention_matrices = [(src_mtx, tgt_mtx) for src_mtx, tgt_mtx in zip(*attention_matrices)]
        else:
            self.attention_matrices = None
        self.span_prediction = span_prediction
        self.mt_with_spans = len(self.pos_list) == 3

        # if target sentences all have length of 1, they are all labels for sequence classification task
        if len(pos_list) == 2 and sum(pos_list[1][:,1] - pos_list[1][:,0]) == len(pos_list[1]):
            self.seq_classif = True
        else:
            self.seq_classif = False
    ...
    def check(self):
        ...
        for i, (pos, sent) in enumerate(zip(self.pos_list, self.sent_list)):
            ...
            if self.span_prediction or self.seq_classif:
                break
            if i == 1:
                break
        ...
        if self.has_attention:
            assert len(self.attention_matrices) == len(self.pos_list[0])
            # first index -> first example, second index is the attention matrix corresponding to the target language
            # so first is the matrix for the source language as the target
            # the second is for the target matrix as the target
            # and then for each matrix, the rows (dim = 0) corresponds to the target lang, dim = 1 to the source lang
            assert len(self.attention_matrices[0]) == len(self.attention_matrices[1]) == 2
            # src_mtx tgt dim = length of the source sentence
            # src_mtx tgt dim = length of the source sentence
            ## CURRENTLY IT LOOKS LIKE THE SOURCE IS 2 GREATER, AND THE TARGET IS ONLY 1 GREATER
            print(f"{self.attention_matrices[0][0].shape[0]} != {self.lengths_list[0][0]}")
            # src_mtx src dim = length of the target sentence
            print(f"{self.attention_matrices[0][0].shape[1]} != {self.lengths_list[1][0]}")
            # tgt_mtx tgt dim = length of the target sentence
            print(f"{self.attention_matrices[0][1].shape[0]} != {self.lengths_list[1][0]}")
            # tgt_mtx src dim = length of the source sentence
            print(f"{self.attention_matrices[0][1].shape[1]} != {self.lengths_list[0][0]}")
            # src_mtx tgt dim = length of the source sentence
            # assert self.attention_matrices[0][0].shape[0]-1 == self.lengths_list[0][0], f"{self.attention_matrices[0][0].shape[0]} != {self.lengths_list[0][0]}"
            # # src_mtx src dim = length of the target sentence
            # assert self.attention_matrices[0][0].shape[1]-2 == self.lengths_list[1][0], f"{self.attention_matrices[0][0].shape[1]} != {self.lengths_list[1][0]}"
            # # tgt_mtx tgt dim = length of the target sentence
            # assert self.attention_matrices[0][1].shape[0]-1 == self.lengths_list[1][0], f"{self.attention_matrices[0][1].shape[0]} != {self.lengths_list[1][0]}"
            # # tgt_mtx src dim = length of the source sentence
            # assert self.attention_matrices[0][1].shape[1]-2 == self.lengths_list[0][0], f"{self.attention_matrices[0][1].shape[1]} != {self.lengths_list[0][0]}"
    ...
    def remove_empty_sentences(self):
        ...
        self.lengths_list = [pos[:, 1] - pos[:, 0] for pos in self.pos_list]
        if self.has_attention:
            self.attention_matrices = [self.attention_matrices[i] for i in indices]
        logger.info("Removed %i empty sentences." % (init_size - len(indices)))
        self.check()
    ...
    def get_batches_iterator(self, batches, return_indices):
        ...
        for sentence_ids in batches:
            ...
            if self.seq_classif:  # sentence classification, the second sentence is the label, do not split
                split_sentences_id[1] = False
            sents = [
                self.batch_sentences([sent[a:b] for a, b in pos], split_id)
                for split_id, pos, sent in zip(split_sentences_id, pos, self.sent_list)
            ]
            # will not work for the case where we have mt_with_spans
            assert len(sents) == 2, "Assuming len(sents) == 2, but got %i" % len(sents)
            if self.has_attention:
                assert len(self.attention_matrices[0]) == 2, "Assuming len(self.attention_matrices) == 2, but got %i" % len(self.attention_matrices)
                attns_src_tgt = [self.attention_matrices[_id] for _id in sentence_ids] # [(src1, tgt1) ... ]
                src_attns, tgt_attns = zip(*attns_src_tgt)
                src_attns_masks, tgt_attns_masks = self.batch_matrices(src_attns), self.batch_matrices(tgt_attns)
                # attns = [[attn_list[_id] for _id in sentence_ids] for attn_list in self.attention_matrices]
                # attns_masks = [self.batch_matrices(attn) for attn in attns] # yields [(attn, mask), ...]
                # assert len(attns_masks) == 2, "Assuming len(attn_masks) == 2, but got %i" % len(attns_masks)
                assert len(src_attns_masks) == len(sents), "Assuming len(src_attns_masks) == len(sents), but got %i and %i" % (
                    len(src_attns_masks), len(sents)
                )
                assert len(tgt_attns_masks) == len(
                    sents), "Assuming len(tgt_attns_masks) == len(sents), but got %i and %i" % (
                    len(tgt_attns_masks), len(sents)
                )
                assert not return_indices, "return_indices is not supported with attention"
                # (src_sent, src attn), (tgt_sent, tgt attn)
                sents = [(sents[0],  src_attns_masks), (sents[1], tgt_attns_masks)]
