...
MAX_VIRTUAL_MEMORY_JAVA = 500 * 1024 * 1024  # ~ 500 MB
...
def shuf_order(langs, params=None, n=5):
    ...
    for l in langs:
        ...
        if len(l) == 1 or l[1] is None:
            ...
            if params.train_splits:
                for t_splt in params.train_splits:
                    para.append((*l, t_splt))
            else:
                para.append(l)
    ...
    if params.lg_sampling_factor == -1:
        ...
        # TODO need to modify params.para_probs as well
...
def convert_to_text(batch, lengths, dico, params, generate_several_reps=False):
    ...
    for j in range(bs):
        ...
        for rep in range(nb_repetitions):
            ...
            sent = " ".join(words)
            sent = SpanAnnotator.remove_spans(sent)
            sentences[j].append(sent)
...
def get_java_compilation_errors(code, timeout=20):
    ...
    comp_cmd = f"{limit_virtual_memory(MAX_VIRTUAL_MEMORY_JAVA)}; {os.path.join(get_java_bin_path(), 'javac')} {file}"
    #print(comp_cmd)
    #comp_cmd_2= f"{limit_virtual_memory(MAX_VIRTUAL_MEMORY)}; {os.path.join(get_java_bin_path(), 'javac')} {f}"
    #print(comp_cmd_2)
    timed_out = False
    try:
        proc = subprocess.run(
...
def get_java_bin_path():
    JAVA_HOME = "../java_8/ibm-java-ppc64le-80/bin" # "/usr/bin/java" "/public/apps/java/jdk/1.8.0_131/bin/"
...
def get_mixed_langs_id(x1: torch.Tensor, lang1_id: int, lang2_id: int, sep_index: int):
    # for each sentence, we look for special separation token (i.e. <special0>) and fill what comes
    # after the sep token as lang2
    # pdb.set_trace()
    bs = x1.shape[1]
    langs_id = x1.clone().fill_(lang1_id)
    sep_indices = torch.where(x1==sep_index)
    assert len(sep_indices[0]) == bs  # assume special separation token only occurs once per sentence

    for match_idx in range(bs):
        langs_id[sep_indices[0][match_idx]:, sep_indices[1][match_idx]] = lang2_id
    return langs_id


def remove_prefix(hypothesis: List[List[str]]):
    new_hypothesis = []
    for i, hyp_list in enumerate(hypothesis):
        new_hyp_list = []
        for hyp in hyp_list:
            if hyp.startswith("<special"):
                hyp = hyp.replace("<special1>", "", 1).replace("<special2>", "", 1)
            new_hyp_list.append(hyp)
        new_hypothesis.append(new_hyp_list)
    return new_hypothesis


def get_sentence_id_len(s, dico):
    sep_idx = dico.index("|")
    l = 1
    while s[l] != sep_idx:
        l += 1
...
def add_dataset_prefix(data_path:str, prefix: str, return_data=False, has_sentence_id=False):
    prefix2idx = {"<special1>":5, "<special2>":6}
    data = torch.load(data_path)
    prefix_idx = np.array([prefix2idx[prefix]], dtype=data["sentences"].dtype)
    offset = 0
    new_sentences = deepcopy(data["sentences"])
    for i in range(len(data["positions"])):
        if has_sentence_id:
            id_len = get_sentence_id_len(new_sentences[data["positions"][i, 0] + offset:], data["dico"])
            new_sentences = np.concatenate([new_sentences[:data["positions"][i, 0] + offset + id_len + 1],
                                            prefix_idx,
                                            new_sentences[data["positions"][i, 0] + offset + id_len + 1:]])
        else:
            new_sentences = np.concatenate([new_sentences[:data["positions"][i,0] + offset],
                                            prefix_idx,
                                            new_sentences[data["positions"][i,0] + offset:]])
        offset += 1
    pos_offset = np.tile(np.arange(0, len(data["positions"])), [2,1]).T
    pos_offset[:, 1] += 1

    data["positions"] += pos_offset
...
def copy_over_training_data_files(params):
    wild_data_path = Path(params.wild_data_path)
    data_path = Path(params.data_path)
    patterns = ["test.*.pth", "valid.*.pth", "train.*.pth"]
    cnt = 0
    print(f"copying train / valid / test splits from data dir {params.data_path}")
    for pattern in patterns:
        for file_path in data_path.glob(pattern):
            new_file_path = wild_data_path.joinpath(file_path.name)
            shutil.copyfile(str(file_path), str(new_file_path))
            cnt += 1
            if params.add_dataset_prefix:
                add_dataset_prefix(
                    str(new_file_path),
                    prefix="<special1>" if "train" in pattern else params.eval_prefix,
                    has_sentence_id=not "train" in pattern
                )

    # files = ["python_translated_tests.json"]
    # for f in files:
    #     shutil.copyfile(data_path.joinpath(f), wild_data_path.joinpath(f))
    #     cnt += 1

    print(f"Moved {cnt} files from {data_path} to {wild_data_path}. ")
    if not params.train_splits:
        if params.wild_data_only:
            print(f"using only wild data for training...")
            for file_path in tqdm(wild_data_path.glob("wild.*.pth")):
                shutil.copy(str(file_path), str(file_path).replace("wild.", "train."))
                if params.add_dataset_prefix:
                    add_dataset_prefix(str(file_path).replace("wild.", "train."), "<special2>")
        else:
            print(f"combining them into train split now...")
            for file_path in tqdm(wild_data_path.glob("train.*.pth")):
                wild_path = str(file_path).replace("train.", "wild.")
                if params.add_dataset_prefix:
                    # load into memory and modify, so don't modify files directly causing downstream issues
                    wild_data = add_dataset_prefix(wild_path, "<special2>", return_data=True)
                else:
                    wild_data = torch.load(wild_path)
                combine_data_splits_binaries([str(file_path), wild_data], file_path)


def combine_data_splits_binaries(input_paths_or_data: List[Union[str, Dict[str, Any]]], output_path: str):
    data_list = [p if isinstance(p, dict) else torch.load(p) for p in input_paths_or_data]
    combined_data = deepcopy(data_list[0])
    sentences_dtype = data_list[0]["sentences"].dtype
    unk_word_cnt = collections.Counter(combined_data["unk_words"])
    for data in data_list[1:]:
        new_data_pos = data["positions"] + combined_data["positions"].max() + 1
        combined_data["positions"] = np.vstack([combined_data["positions"], new_data_pos])
        combined_data["sentences"] = np.append(combined_data["sentences"], data["sentences"])
        unk_word_cnt.update(data["unk_words"])
    combined_data["unk_words"] = dict(unk_word_cnt)
    combined_data["sentences"] = combined_data["sentences"].astype(sentences_dtype)
    torch.save(combined_data, output_path, pickle_protocol=4)


def combine_data_splits_bpe(input_paths, output_path):
    combined_data = ""
...
def combine_train_splits(params):
    # mostly for QE model training
    data_path = Path(params.data_path)

    for file_path in data_path.glob("train.*.pth"):  # avoid partitioned splits
        print(f"combining splits {params.train_splits} for {file_path}")
        input_paths = [str(file_path).replace("train", s) for s in params.train_splits]
        combined_path = str(file_path).replace("train", "train_combined")
        combine_data_splits_binaries(input_paths, combined_path)

    if params.seq_classif_steps:
        for file_path in data_path.glob("train.*.qe_label.bpe"):
            input_paths = [str(file_path).replace("train", s) for s in params.train_splits]
            combined_path = str(file_path).replace("train", "train_combined")
            combine_data_splits_bpe(input_paths, combined_path)


def shift_paddings_for_qe(x1, x3, pad_index, sep_index):
    num_paddings = (x1==pad_index).sum(dim=0)
    x1_len = x1.shape[0]
    for i in range(x3.shape[1]):
        new_target = torch.cat([torch.tensor([sep_index], dtype=x1.dtype, device=x1.device),
                                x3[x1_len+2:, i],
                                torch.ones(num_paddings[i]+2, dtype=x1.dtype, device=x1.device)*pad_index])
        x3[x1_len-num_paddings[i]-1:, i] = new_target
    return x3