...
from ..data.loader import DATASET_SPLITS, TRAIN_SPLITS
...
TARGET_ST_LANG =  {"python"} #{"cpp", "python"}
...
EVAL_DATASET_SPLITS = [ds for ds in DATASET_SPLITS if ds not in TRAIN_SPLITS]
...
class Evaluator(object):
    def __init__(self, trainer, data, params):
        ...
        self.dico = data["dico"]
        self.params = params
        EVAL_DATASET_SPLITS.extend(params.eval_splits)
        ...
        if self.params.eval_st:
            ...
            self.evosuite_tests_dico = load_evosuite_transcoder_tests(params)
        else:
            self.evosuite_tests_dico = None

        assert self.params.n_jobs > 0 and self.params.n_workers > 0
        self.parallel_eval = (self.params.n_jobs > 1 and self.params.n_workers > 1)
    ...
    def get_iterator(
        ...
        assert (
            ...
            or (lang1, lang2, label) in self.params.seq_classif_steps
        )
        ...
        if lang2 is None or lang2 == lang1:
            ...
            key = (_lang1, _lang2, span) if span is not None else \
                  (_lang1, _lang2, label) if label is not None else (_lang1, _lang2)
        ...
        for batch in iterator:
            batch = batch if lang2 is None or lang1 == lang2 or lang1 <= lang2 or label is not None else batch[::-1]
            ...
            if self.params.add_dataset_prefix:
                x2 = torch.cat([batch[1][0][[0]], batch[1][0][2:]])  # remove prefix from target sentence
                len2 = batch[1][1] - 1  # decrease target sentence len
                batch = (batch[0], (x2, len2, batch[1][2], batch[1][3]))
            yield batch
    ...
    def create_reference_files(self):
        ...
        """
        logger.info("Creating reference files for BLEU evaluation...")
        params = self.params
        for key, v in self.data["para"].items():
            ...
            for data_set in EVAL_DATASET_SPLITS:
                ...
                for i, batch in enumerate(
                    ...
                    logger.info("converting to text for {0}".format(data_set))
                    if len(batch) == 2:
    ...
    def save_qe_prediction_file(self, predictions: Dict[str, List]):
        # pdb.set_trace()
        predict_class_path = os.path.join(self.params.dump_path, "predictions_classes.txt")
        prediction_path = os.path.join(self.params.dump_path, "predictions.csv")
        with open(predict_class_path, "w", encoding="utf-8") as f:
            f.write("\n".join(predictions["TARGET_CLASS"]) + "\n")
        restore_segmentation(
            predict_class_path, roberta_mode=self.params.roberta_mode, single_line=True
        )
        restored_class_ids = [l.strip() for l in open(predict_class_path).readlines()]
        predictions["TARGET_CLASS"] = restored_class_ids
    ...
    def run_all_evals(self, trainer):
        ...
        """
        if self.parallel_eval:
            trainer.save_checkpoint(name=self.eval_model_name,include_optimizers=False)
        ...
        with torch.no_grad():
            ...
            for data_set in EVAL_DATASET_SPLITS if not params.eval_splits else params.eval_splits: #["test"]:
                ...
                for keys in set(
                    ...
                    if len(keys) == 3:
                        spans = keys[2]
                    # if (lang1 == "java" or lang1 == "java_sa" or lang1 == "java_obfuscated") and \
                    #    (lang2 == "python" or lang2 == "python_sa" or lang2 == "python_obfuscated"):
                    # only evaluate on directions where we keep validation metrics checkpoints
                    if f"{lang1}-{lang2}" in params.validation_metrics:
                        self.evaluate_mt(
                            scores,
                            data_set,
                            lang1,
                ...
                # token-wise prediction task (evaluate perplexity and accuracy)
                for lang1, lang2 in params.classif_steps:
                    self.evaluate_classif(scores, data_set, lang1, lang2)

                # sequence prediction task (evaluate perplexity and accuracy)
                for lang1, lang2, label in params.seq_classif_steps:
                    self.evaluate_classif(scores, data_set, lang1, lang2, label)

                # report average metrics per language
                if self.params.eval_do and len(params.do_steps) > 0 and params.is_master:
    ...
    def evaluate_classif(self, scores, data_set, lang1, lang2, label=None):
        params = self.params
        assert data_set in EVAL_DATASET_SPLITS
        assert lang1 in params.langs
        lang1_id = params.lang2id[lang1]
        ...
        predictions = {f"logit{cl}":[] for cl in range(self.params.n_classes_classif)}
        predictions["TARGET_CLASS"]= []
        for batch in self.get_iterator(data_set, lang1, lang2, stream=False, label=label):
            if label is not None:
                (x1, len1, id1, lenid1), (y, len2, id2, lenid2) = batch
                pred_mask = torch.zeros(x1.shape[0], x1.shape[1]).bool()
                pred_mask[0, :] = True  # use first token (eos) as sentence token
                assert all(len2 == 3)  # label shape is 3 x n, where y[1,:] holds class index label
                if all(y[1, :] == -1):  # no label, has to be wild set, only need to predict and save logits
                    y = None
                    predictions["TARGET_CLASS"].extend(convert_to_text(id1, lenid1, self.dico, params))
            else:
            ...
            # if no label provided, save word_scores and return
            if y is None:
                for cl in range(self.params.n_classes_classif):
                    predictions[f"logit{cl}"].extend(word_scores[:, cl].cpu().numpy().tolist())
                continue

            # update stats
            y_ = y[pred_mask].view(-1,) if label is None else y[1,:].view(-1,)
            n_words += len(y_)
        ...
        # if no labels, save predictions and exit
        if len(predictions["TARGET_CLASS"]) > 0:
            # pdb.set_trace()
            self.save_qe_prediction_file(predictions)
            return
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(
        ...
        if params.eval_bleu_test_only:
            ...
            datasets_for_bleu = [s for s in EVAL_DATASET_SPLITS if s not in TRAIN_SPLITS] #  "test"
        ...
        decoder = (
            ...
            self.decoder[params.train_splits.index("wild")] if params.separate_dataset_decoders else
            self.decoder[0]
        )
        ...
        for deobfuscation_proba in (
            ...
            if deobfuscate:
                rng = np.random.RandomState(0)
            hypothesis, n_valid, n_words, references, sources, xe_loss = self.decode_and_test_data(data_set,
                                                                                                   datasets_for_bleu,
                                                                                                   decoder, deobfuscate,
                                                                                                   deobfuscation_proba,
                                                                                                   encoder, eval_bleu,
            ...
            if eval_computation and data_set in datasets_for_bleu and data_set != "test_st":
            ...
            if (
                eval_st
                and data_set in ("test_st") #datasets_for_bleu
    ...
    def decode_and_test_data(self, data_set, datasets_for_bleu, decoder, deobfuscate, deobfuscation_proba, encoder,
                             eval_bleu, eval_computation, eval_subtoken_score, lang1, lang1_id, lang2, lang2_id, params,
                             rng, span, torch_rng, n_jobs=1, n_workers=1):

        if self.parallel_eval:
            ## swap decoder and encoder with None (because instead we will reload it)
            return self.decode_and_test_data_parallel_driver(data_set, datasets_for_bleu, None, deobfuscate,
                                                             deobfuscation_proba, None, eval_bleu, eval_computation,
                                                             eval_subtoken_score, lang1, lang1_id, lang2, lang2_id,
                                                             params, rng, span, torch_rng, n_jobs, n_workers)
        else:
            return self.decode_and_test_data_single(data_set, datasets_for_bleu, decoder, deobfuscate,
                                                    deobfuscation_proba, encoder,eval_bleu, eval_computation,
                                                    eval_subtoken_score, lang1, lang1_id, lang2,  lang2_id,
                                                    params, rng, span, torch_rng)

    def decode_and_test_data_single(self, data_set, datasets_for_bleu, decoder, deobfuscate, deobfuscation_proba, encoder,
                             eval_bleu, eval_computation, eval_subtoken_score, lang1, lang1_id, lang2, lang2_id, params,
                             rng, span, torch_rng):
        n_words = 0
        xe_loss = 0
        ...
        return hypothesis, n_valid, n_words, references, sources, xe_loss

    def decode_and_test_data_parallel_worker(self,data_set, datasets_for_bleu, decoder, deobfuscate, deobfuscation_proba,
                                      encoder, eval_bleu, eval_computation, eval_subtoken_score, lang1, lang1_id,
                                      lang2, lang2_id, params, rng, span, torch_rng, worker_id, device_no):

        #### Reload Model ####
        reloaded = torch.load(os.path.join(self.params.dump_path, (self.eval_model_name + ".pth")), map_location="cpu")
        ...
        print("Building model with device number {}...".format(device_no), flush=True, file=sys.stderr)
        gpu = torch.cuda_is_available() if device_no is None else False
        ...
        for i, batch in enumerate(
            ...
            assert len(batch) >= 2
            if len(batch) == 2:
                raise NotImplementedError("not supported in parallel mode yet")
                if lang1 == lang2:
                    x2, len2 = batch
                    x1, len1 = add_noise(
                        x2,
            ...
            # cuda
            device_id = f"cuda:{device_no}"
            x1, len1, langs1, x2, len2, langs2, y, spans = to_device(
                x1, len1, langs1, x2, len2, langs2, y, spans, device=device_id
            )
            # encode source sentence
            enc1 = encoder(
                "fwd", x=x1, lengths=len1, langs=langs1, causal=False, spans=spans
    ...
    def decode_and_test_data_parallel_driver(self, data_set, datasets_for_bleu, decoder, deobfuscate, deobfuscation_proba,
                                      encoder, eval_bleu, eval_computation, eval_subtoken_score, lang1, lang1_id,
                                      lang2, lang2_id, params, rng, span, torch_rng, n_jobs, n_workers):
        ...
        for i in range(n_jobs):
            device_no = (i & n_workers)
            # do not pass down the encoder or the decoder
            jobs.append((
                    data_set,
                    datasets_for_bleu,
                    None, #decoder.to("cpu"),
        ...
        with mp.Pool(n_jobs) as pool:
            for job in tqdm(pool.starmap_async(self.decode_and_test_data_parallel_worker, jobs).get()):
                hypothesis.extend(job[0])
                n_valid += job[1]
                n_words += job[2]
                references.extend(job[3])
                sources.extend(job[4])
                xe_loss += job[5]
    ...
    def write_hypo_ref_src(
        ...
        # remove any prefix in reference if present before saving to hypothesis file (for dataset prefix runs)
        hypothesis = remove_prefix(hypothesis)
    ...
    def compute_comp_acc(
        ...
        eval_script_dir = EVAL_SCRIPT_FOLDER[data_set]
        if params.eval_fixed_data:
            eval_script_dir = Path(str(eval_script_dir)+"_fixed")
        ...
        total_evaluated = func_run_stats["total_evaluated"] if func_run_stats["total_evaluated"] else 1
        scores[
            "%s_%s-%s_mt_comp_acc" % (data_set + prefix, lang1, lang2)
        ] = func_run_stats["success"] / total_evaluated
        scores["%s_%s-%s_mt_failure" % (data_set + prefix, lang1, lang2)] = func_run_stats["failure"] / total_evaluated
        scores["%s_%s-%s_mt_error" % (data_set + prefix, lang1, lang2)] = func_run_stats["error"] / total_evaluated
        scores["%s_%s-%s_mt_gold" % (data_set + prefix, lang1, lang2)] = func_run_stats["identical_gold"] / total_evaluated
        scores["%s_%s-%s_mt_timeout" % (data_set + prefix, lang1, lang2)] = func_run_stats["timeout"] / total_evaluated
