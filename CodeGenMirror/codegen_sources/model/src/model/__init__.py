...
from .transformer import DECODER_ONLY_PARAMS, TransformerModel, Classifier, SequenceClassifier
...
@torch.no_grad()
def build_model(params, dico, gpu=True, device_no=None, qe_model_path=None):
    """
    Build model.
    """
    if params.encoder_only or qe_model_path:
        # build
        ...
        # reload a pretrained model
        if qe_model_path:
            logger.info("============ QE Model Reloading")
            logger.info("Reloading QE model from %s ..." % qe_model_path)
            reload_transformer(params, qe_model_path, dico, model, "model", gpu)
        ...
        logger.info("")
        if params.freeze_encoder or qe_model_path:
            for param in model.parameters():
                param.requires_grad = False
            if qe_model_path:
                model.eval()
            logger.info(f"{'Encoder' if params.freeze_encoder else 'QE model'} parameters set to frozen")
        ...
        elif hasattr(params, "separate_dataset_decoders") and params.separate_dataset_decoders:
            decoders = [
                TransformerModel(params, dico, is_encoder=False, with_output=True)
                for _ in params.train_splits
            ]
        else:
            decoders = [
                TransformerModel(params, dico, is_encoder=False, with_output=True)
        ...
        if device_no is not None:
            print("Device: {}".format(device_no), flush=True, file=sys.stderr)
            result = (
            [encoder.to(f"cuda:{device_no}")],
            [dec.to(f"cuda:{device_no}")],
        )
        else:
            print("Device was None!", flush=True, file=sys.stderr)
            result = (
...
@torch.no_grad()
def build_classifier(params, gpu=True, qe_model_path=None):
    """
    Build classifier.
    """
    # build
    if qe_model_path:
        params.n_classes_classif = 2
    classifier = SequenceClassifier(params) if (len(params.seq_classif_steps) > 0) or qe_model_path else Classifier(params)

    # reload a pretrained model
    reload_path = qe_model_path if qe_model_path else params.reload_classifier
    if reload_path:
        logger.info("Reloading classifier from %s ..." % reload_path)
        reloaded = torch.load(
            reload_path,
        ...
        if "classifier" not in reloaded:
            logger.warning(
                f"There is no classifier in {reload_path}. The classifier weights will be initialized randomly"
            )
        else:
    ...
    return [classifier.cuda() if gpu else classifier]
...
def reload_transformer(params, path, dico, model, model_type, gpu=True):
    ...
    print(reloaded.keys(), model_type)
...
def clean_model_state_dict(reloaded, model_type):
    ...
    model_reloaded = reloaded[model_type if model_type in reloaded else "encoder"]  # else "model"
