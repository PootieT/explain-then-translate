...
LAYER_NORM_EPSILON = 1e-12  # according to https://github.com/facebookresearch/CodeGen/blob/main/docs/transcoder.md
# this would ensure output to match exactly like transcoder
...
class Classifier(nn.Module):
    """
    Classifier layer (cross_entropy loss). token-wise classification
    """
...
class SequenceClassifier(nn.Module):
    ...
    source: huggingface modeling_plbart.py"""
    ...
    def __init__(self, params):
        ...
        self.dropout = nn.Dropout(p=params.classifier_dropout)
        self.proj = nn.Linear(self.emb_dim, self.n_classes)
        if params.weighted_loss:
            self.class_weights = torch.tensor(params.class_weights, requires_grad=False).float()
        else:
            self.class_weights = None

    def forward(self, x: torch.Tensor, y: torch.Tensor, pred_mask):
        """ input full sequence, as well as pred_mask, which should be only the BOS token """
        device = x.device
        x = x[pred_mask.unsqueeze(-1).expand_as(x)].view(-1, self.emb_dim)
        ...
        scores = x.view(-1, self.n_classes)
        assert sum(sum(pred_mask.int())).item() == scores.shape[0]
        if y is None:
            return scores, None
        y = y[1, :].view(-1, )
        if self.n_classes == 1:
            if self.class_weights is not None:
                self.class_weights = self.class_weights.to(device) == y
            loss = F.binary_cross_entropy_with_logits(
                scores.float().squeeze(1), y.float(), reduction="mean", weight=self.class_weights).type_as(scores)
        else:
            if self.class_weights is not None and self.class_weights.device != device:
                self.class_weights = self.class_weights.to(device)
            loss = F.cross_entropy(scores.float(), y, reduction="mean", weight=self.class_weights).type_as(scores)
        return scores, loss
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input, mask, kv=None, use_cache=False):
        ...
        # cache attention probabilities
        if cross_attn:
            # print("layer:", self.layer_id, "scores shape", scores.shape)
            # print("self.scores == None: ", self.scores==None)
            if self.scores != None:
                if self.scores.get(self.layer_id) != None:
                    # print("layer:", self.layer_id, "dim shape", self.scores[self.layer_id].shape, "scores shape", weights.shape)
                    # self.scores[self.layer_id] = scores
                    self.scores[self.layer_id] = torch.cat((self.scores[self.layer_id], weights), dim=2)
                else:
                    self.scores[self.layer_id] = weights
...
class TransformerModel(nn.Module):
    ...
    def forward(self, mode, **kwargs):
        ...
        """
        if mode == "fwd":
            ##TODO REMOVE THIS FOR TRAINING -- fix root cause
            if self.scores != dict():
                del self.scores
                self.scores = dict()
        ...
        elif mode == "attention_loss":
            return self.calc_attn_loss(**kwargs)
        else:
    ...
    def fwd(
        ...
        for i in range(self.n_layers):
            ...
            # pdb.set_trace()
            # if hasattr(self, "layer_cache") and self.layer_cache is not None:
            #     self.layer_cache[i] = attn
            ...
            if self.is_decoder and src_enc is not None:
                ...
                # pass on "global" scores dict to each of the individual cross attention layers
                self.encoder_attn[i].scores = self.scores
    ...
    def _get_attention_tensor(self):
        """
        Scores is a dict of layeridx -> tensor, where tensor has shape (bsz, n_heads, output, input)
        Returns tensor of shape (bs, layers, n_heads, output, input)
        """
        assert self.scores is not None and len(self.scores) == self.n_layers, "No scores available"
        scores_tensor = torch.stack(list(self.scores.values()), dim=1) # cat on dim 1 for layers
        return scores_tensor

    def calc_attn_loss(self, target_scores, target_mask, loss_type = "mse", broadcast_mask: torch.Tensor = None):
        """
        target_scores: (bs, output, input)
        target_mask: (bs, output, input)
        broadcast mask: binary tensor of shape (layers, n_heads), mask out target mask for layers and heads
        broadcasts to (bs, layers, n_heads, output, input)
        """
        assert loss_type in ["xent", "mse", "kl"], "Only xent, mse, kl supported"
        if loss_type == "xent":
            ...
            loss_fn = F.kl_div
        neural_attention_scores = self._get_attention_tensor()
        # broadcast attention scores
        tensor_type = neural_attention_scores.dtype
        target_scores = target_scores.unsqueeze(1).unsqueeze(1).expand_as(neural_attention_scores).type(tensor_type).clone()
        target_mask = target_mask.unsqueeze(1).unsqueeze(1).expand_as(neural_attention_scores).type(tensor_type).clone()
        if broadcast_mask is not None:
            broadcast_mask = broadcast_mask.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand_as(neural_attention_scores).type(tensor_type).clone()
            # anywhere that was a 1, we keep, anywhere that was a 0, we zero out
            target_mask = target_mask * broadcast_mask
        neural_attention_scores *= target_mask
        target_scores *= target_mask
        assert neural_attention_scores.shape == target_scores.shape
        loss = loss_fn(neural_attention_scores, target_scores, reduction="mean")
        return loss
